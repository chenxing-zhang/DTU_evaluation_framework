...
list_tab_anno_barcode = list()
for(dataSubType in c('GSE153859_CBS1', 'GSE153859_CBS2', 'GSE153859_MOB')){
  seu = read_rds(paste(path_data,paste0(dataSubType,'.rds'),sep='/'))
  colnames(seu) = gsub('-| |/|\\(|\\)|\\+','_',colnames(seu))
  
  # load expression
  tab_count_trans = as.data.frame(seu@assays[["ISO"]]@counts)
  vec_trans_ori = rownames(seu@assays[["ISO"]]@counts)
  vec_barcode = colnames(seu@assays[["ISO"]]@counts)
  vec_trans = unlist(lapply(vec_trans_ori, function(x) unlist(strsplit(x,'\\.\\.'))[2]))
  tab_count_trans['transcript_id'] = vec_trans
  
  # merge trans information
  tab_anno_trans_ori = read.table('~/data/Ref_mouse/GENECODE_M24/tab_anno_trans',header=T,sep='\t')
  tab_anno_trans = tab_anno_trans_ori[,c('transcript_id','gene_id')]
  tab_count_trans = merge(tab_count_trans,tab_anno_trans,by='transcript_id',all.x=T)
  tab_count_trans = tab_count_trans[,c(c('transcript_id','gene_id'),vec_barcode)]
  rownames(tab_count_trans) = tab_count_trans$transcript_id
  colnames(tab_count_trans) = c(c('feature_id','gene_id'),vec_barcode)
  tab_count_trans = tab_count_trans %>% filter(!is.na(gene_id))
  
  # load sample information
  tab_anno_barcode = seu@meta.data
  
  # load terms
  vec_term = sort(as.character(unique(tab_anno_barcode$ClusterName)))
  
  list_vec_term[[dataSubType]] = vec_term
  list_tab_count_trans[[dataSubType]] = tab_count_trans 
  list_tab_anno_barcode[[dataSubType]] = tab_anno_barcode
}
...
DTU_multicore <-function(combin){
  ...
  if (dataSubType == 'GSE153859_CBS2' && term1 == 'Hippocampus area' && term2 == 'Isocortex-1') return(0)
  if (dataSubType == 'GSE153859_CBS2' && term1 == 'Isocortex-1' && term2 == 'Midbrain') return(0)
  
  # load expression and sample information
  tab_count_trans  = list_tab_count_trans[[dataSubType]]
  tab_anno_barcode = list_tab_anno_barcode[[dataSubType]]
  
  # filter samples
  vec_sample1 = sort(rownames(tab_anno_barcode %>% filter(ClusterName== term1)))
  vec_sample2 = sort(rownames(tab_anno_barcode %>% filter(ClusterName== term2)))
  list_samples = c(vec_sample1,vec_sample2)
  
  # filter expression
  tab_count = tab_count_trans[,c('feature_id','gene_id',list_samples)]
  tab_count = tab_count[rowSums(tab_count[,-c(1:2)]>=2)>=3,]# filter, important !!
  ...
  # SPIT
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'SPIT',sep='_'),sep='/'))){
    ...
    tab_time_SPIT = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'SPIT',time = all_time)
    write.table(tab_time_SPIT, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'SPIT',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_SPIT, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'SPIT',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # SUPPA2
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'SUPPA2',sep='_'),sep='/'))){
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_SUPPA2 = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'SUPPA2',time = all_time)
    write.table(tab_time_SUPPA2, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'SUPPA2',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_SUPPA2, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'SUPPA2',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # RATs
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU', term1_change,term2_change,'RATs',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_RATs = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'RATs',time = all_time)
    write.table(tab_time_RATs, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'RATs',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_RATs, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'RATs',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  ################################################################################
  # DTUrtle
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DTUrtle',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_DTUrtle = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'DTUrtle',time = all_time)
    write.table(tab_time_DTUrtle, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'DTUrtle',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_DTUrtle, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DTUrtle',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # NBSplice
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'NBSplice',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_NBSplice = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'NBSplice',time = all_time)
    write.table(tab_time_NBSplice, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'NBSplice',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_NBSplice, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'NBSplice',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # limma-DTU
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'limma',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_limma = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'limma',time = all_time)
    write.table(tab_time_limma, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'limma',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_limma, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'limma',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # edgeR-DTU
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'edgeR',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_edgeR = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'edgeR',time = all_time)
    write.table(tab_time_edgeR, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'edgeR',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_edgeR, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'edgeR',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }

  # satuRn
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'satuRn',sep='_'),sep='/'))){
    ...
    invisible({
      ...
      end_time <- Sys.time()
      all_time = as.numeric(end_time) - as.numeric(start_time)
      tab_time_satuRn = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'satuRn',time = all_time)
      write.table(tab_time_satuRn, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'satuRn',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
      write.table(tab_result_satuRn, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'satuRn',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    })
  }
  
  ################################################################################
  # DRIMSeq 
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DRIMSeq',sep='_'),sep='/'))){
    start_time <- Sys.time()
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_DRIMSeq = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'DRIMSeq',time = all_time)
    write.table(tab_time_DRIMSeq, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'DRIMSeq',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_DRIMSeq, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DRIMSeq',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }
  
  # DEXSeq
  if(!file.exists(paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DEXSeq',sep='_'),sep='/'))){
    start_time <- Sys.time()
    group <- relevel(factor(tab_label$group),ref=term1_change)
    ...
    end_time <- Sys.time()
    all_time = as.numeric(end_time) - as.numeric(start_time)
    tab_time_DEXSeq = data.frame(dataType=dataType,dataSubType=dataSubType,term1=term1_change,term2=term2_change,m_DTU = 'DEXSeq',time = all_time)
    write.table(tab_time_DEXSeq, paste(path_save,dataSubType,paste('tab_time',term1_change,term2_change,'DEXSeq',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
    write.table(tab_result_DEXSeq, paste(path_save,dataSubType,paste('tab_DTU',term1_change,term2_change,'DEXSeq',sep='_'),sep='/'), row.names = TRUE,sep="\t",quote=FALSE)
  }
