...
def convert_counts_to_IF(counts): 
...
def compute_double_stats(ifs, gene_counts, tx2gene_dict, ctrl, case, cluster_size_lim, perm_p_cutoff, b, f_cpm):
    ...
    vec_trans = ifs.index.to_list()
    for i in vec_trans:
        try:
            selected_ctrls, ctrl_arr, filtered_ctrls = filter_samples_on_gene_cpm(f_cpm, i, tx2gene_dict, ctrl, ifs,
                                                                                  cpms)
            selected_cases, case_arr, filtered_cases = filter_samples_on_gene_cpm(f_cpm, i, tx2gene_dict, case, ifs,
                                                                                  cpms)
            ...
            likelihood_arr.append(0)
            wrst_p_arr.append(1)
    return vec_trans, np.array(likelihood_arr), np.array(wrst_p_arr), genotype_cluster_df
...
def compute_double_stats_zcx(counts, tx2gene, ctrl_samples, case_samples):
	IFs, gene_counts = convert_counts_to_IF(counts)
	tx2gene_dict = tx2gene.gene_id.to_dict()
	args_n_small = 1
	args_p_cutoff = 1
	args_bandwidth = 0.09
	args_f_cpm = 0
	vec_trans, likelihood_arr, wrst_p_arr, genotype_cluster_df = compute_double_stats(IFs, gene_counts, tx2gene_dict, ctrl_samples, case_samples, args_n_small, args_p_cutoff, args_bandwidth, args_f_cpm)
	vec_gene = [tx2gene_dict[trans] for trans in vec_trans]
	return vec_trans,vec_gene, likelihood_arr, wrst_p_arr, genotype_cluster_df
